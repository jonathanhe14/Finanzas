  ...
  return (
    <div className="kpi-card bg-white border border-border rounded-2xl p-5 shadow-card animate-fade-up overflow-hidden relative">
      <div className="flex items-start justify-between mb-4">
        <div className={`w-9 h-9 rounded-xl ${trending ==="up"? "bg-green-100":"bg-rose-50"} flex items-center justify-center`}>
          {trending === "up" ? (
      ...
      <div className="flex items-center gap-1.5">
        <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-rose-50 text-rose-600">
          {porcentaje > 0 ? `+${porcentaje.toFixed(2)}%` : `${porcentaje.toFixed(2)}%`}
        </span>
