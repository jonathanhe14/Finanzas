...
export default function MovimientosA() {
  const {
    registros,
    allRegistros,
    loading,
  ...
  const handleCreateSave = async (newRegistro) => {
    const { error } = await createRegistro(newRegistro);
    if (!error) {
      handleClose();
    } else {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!loading && !error && allRegistros.length > 0 && (
          ...
          <StatsBar registros={registros} onClick={handleCreate} />
        )}
